...
{
	struct tq_ent *ent, *iter, *pos, *n;
	struct list_head *head, member;
	...
	for (pos = list_entry((head)->next, tq_ent, q_node), n = list_entry(pos->q_node.next, tq_ent, q_node);
	     &pos->q_node != (head); 					\
	     pos = n, n = list_entry(n->q_node.next, tq_ent, q_node))
	{
		list_del(&ent->q_node);
		free(ent);
	}
	...
	memset(tq, 0, sizeof(struct thread_q));	/* poison */
	free(tq);
}
